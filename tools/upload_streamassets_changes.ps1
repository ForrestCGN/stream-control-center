...
if (!(Test-Path $SourceRoot)) { throw "SourceRoot existiert nicht: $SourceRoot" }
...
try {
  ...
  if ($fileCheck) { $fileCheck | Out-Host } else { Write-Host "OK: keine verdächtigen gestagten Dateinamen." }
  ...
  if ($Commit) {
    ...
    if ($Push) {
      ...
      Write-Step "Commit erstellt, aber nicht gepusht. Push mit: git push origin dev"
    }
  } else {
