...
$logPath = "D:\gpt\STEP201_11_DISCORD_CHECK_$timestamp.log"
...
Write-Log "STEP201.11 Discord Datei-, Routen- und API-Pruefung"
...
$discordFiles = @()
try {
  $discordFiles = Get-ChildItem -Recurse backend,htdocs,config,docs,project-state -File -ErrorAction SilentlyContinue |
    Where-Object {
      $_.Name -match 'discord|Discord|voice|Voice|queue|Queue' -or $_.FullName -match 'discord|Discord|voice|Voice|queue|Queue'
    } |
    Select-Object `
...
try {
  $routeSearch = Select-String `
    ...
    -Pattern 'discord','Discord','/api/discord','/discord','voice','Voice','queue','Queue','join','leave' `
    -SimpleMatch |
    Select-Object `
...
$moduleDefs = @(
  ...
  @{ key="discord-legacy"; base="/discord" }
)
...
$targetedProbeUrls = @(
  @{ name="discord api status"; method="GET"; url="http://127.0.0.1:8080/api/discord/status" },
  @{ name="discord api queue status"; method="GET"; url="http://127.0.0.1:8080/api/discord/queue/status" },
  @{ name="discord api sounds"; method="GET"; url="http://127.0.0.1:8080/api/discord/sounds" },
  @{ name="discord legacy status"; method="GET"; url="http://127.0.0.1:8080/discord/status" },
  @{ name="discord legacy queue status"; method="GET"; url="http://127.0.0.1:8080/discord/queue/status" },
  @{ name="discord play missing key"; method="GET"; url="http://127.0.0.1:8080/discord/play" }
)
...
$output = [ordered]@{
  ...
  purpose = "STEP201.11 Discord-Datei-, Routen- und Standard-Endpunkt-Pruefung"
  ...
  discordFiles = [ordered]@{
    ok = [string]::IsNullOrWhiteSpace($fileError)
    error = $fileError
    count = @($discordFiles).Count
    rows = @($discordFiles)
  }
  routeSearch = [ordered]@{
...
Write-Log ""
Write-Log "Discord files: $(@($discordFiles).Count)"
($discordFiles | Format-Table repoPath,length,lastWriteTime -AutoSize | Out-String) | ForEach-Object { Write-Log $_ }
