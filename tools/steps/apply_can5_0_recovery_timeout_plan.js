/* STEP CAN-5.0 - plan recovery and timeout strategy. Documentation only. */
'use strict';
...
function prependOnce(rel, marker, lines) {
  ...
  if (current.includes(marker)) {
    console.log('[CAN-5.0] already present', rel);
    return;
  }
  fs.mkdirSync(path.dirname(abs), { recursive: true });
  fs.writeFileSync(abs, lines.join('\r\n') + '\r\n\r\n' + current, 'utf8');
  console.log('[CAN-5.0] updated', rel);
}
...
writeFile('docs/system-inspection/EVENTBUS_CAN5_0_RECOVERY_TIMEOUT_STRATEGY_PLAN.md', planDoc);
prependOnce('project-state/CURRENT_STATUS.md', 'STEP_CAN5_0_RECOVERY_TIMEOUT_PLAN', currentStatusBlock);
prependOnce('project-state/NEXT_STEPS.md', 'STEP_CAN5_0_NEXT_STEPS', nextStepsBlock);

console.log('[CAN-5.0] done - documentation only, no backend module code changed');
