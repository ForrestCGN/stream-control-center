/* STEP CAN-4.3 - document stable overlay ACK state. No backend module code changes. */
'use strict';
...
function prependOnce(rel, marker, block) {
  ...
  if (current.includes(marker)) {
    console.log('[CAN-4.3] already present', rel);
    return;
  }
  const next = block.replace(/\n/g, '\r\n') + '\r\n' + current;
  fs.mkdirSync(path.dirname(abs), { recursive: true });
  fs.writeFileSync(abs, next, 'utf8');
  console.log('[CAN-4.3] updated', rel);
}

const stableDoc = `# EVENTBUS CAN-4.3 OVERLAY ACK STABLE STATUS
...
const currentStatusBlock = `## STEP CAN-4.3 Overlay ACK stabil dokumentiert
...
const nextStepsBlock = `## Nach STEP CAN-4.3
...
console.log('[CAN-4.3] done - documentation only, no backend module code changed');
