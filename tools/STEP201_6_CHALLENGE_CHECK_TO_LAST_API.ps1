...
$logPath = "D:\gpt\STEP201_6_CHALLENGE_CHECK_$timestamp.log"
...
Write-Log "STEP201.6 Challenge Datei-, Routen- und API-Pruefung"
...
$challengeFiles = @()
try {
  $challengeFiles = Get-ChildItem -Recurse backend,htdocs,config,docs,project-state -File |
    Where-Object {
      $_.Name -match 'challenge|Challenge' -or $_.FullName -match 'challenge|Challenge'
    } |
    Select-Object `
  ...
  $challengeFilesError = $_.Exception.Message
}
...
try {
  $routeSearch = Select-String `
    ...
    -Pattern 'challenge','/api/challenge','/scripts/challenge' `
    -SimpleMatch |
    Select-Object `
...
foreach ($endpoint in $standardEndpoints) {
  ...
  $url = "http://127.0.0.1:8080/api/challenge/$endpoint"
...
$output = [ordered]@{
  ...
  purpose = "STEP201.6 Challenge-Datei-, Routen- und Standard-Endpunkt-Pruefung"
  ...
  challengeFiles = [ordered]@{
    ok = [string]::IsNullOrWhiteSpace($challengeFilesError)
    error = $challengeFilesError
    count = @($challengeFiles).Count
    rows = @($challengeFiles)
  }
  routeSearch = [ordered]@{
...
Write-Log ""
Write-Log "Challenge files: $(@($challengeFiles).Count)"
$challengeFiles | Format-Table repoPath,length,lastWriteTime -AutoSize | Out-String | ForEach-Object { Write-Log $_ }
...
@($summary) | Format-Table key,okCount,total,complete,missing -AutoSize | Out-String | ForEach-Object { Write-Log $_ }
