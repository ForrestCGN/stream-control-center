...
$expectedDocs = @(
  @{
    ...
    markerGroups = @(
      @("STEP596", "MISSING_ROUTES"),
      @("STEP597", "FALSE_POSITIVE"),
      @("STEP599")
    )
  },
  @{
    ...
    markerGroups = @(
      @("STEP601_CURRENT_STATUS_CROSSMODULE_BATCH_START", "STEP601_CURRENT_STATUS_CROSSMODULE_BATCH_END")
    )
  },
  @{
    name = "Channelpoints"
    ...
    markerGroups = @(
      @("STEP603A_CHANNELPOINTS_MODULE_DOCS_BATCH_START", "STEP603A_CHANNELPOINTS_MODULE_DOCS_BATCH_END")
    )
  },
  @{
    ...
    markerGroups = @(
      @("STEP603B_SOUND_SYSTEM_ROUTING_DOCS_BATCH_START", "STEP603B_SOUND_SYSTEM_ROUTING_DOCS_BATCH_END")
    )
  },
  @{
    ...
    markerGroups = @(
      @("STEP605_DASHBOARD_COMMANDS_MODULE_DOCS_BATCH_START", "STEP605_DASHBOARD_COMMANDS_MODULE_DOCS_BATCH_END")
    )
  },
  @{
    ...
    markerGroups = @(
      @("STEP607_COMMUNICATION_BUS_CONTRACT_BATCH_START", "STEP607_COMMUNICATION_BUS_CONTRACT_BATCH_END")
    )
  },
  @{
    ...
    markerGroups = @(
      @("STEP609_SHOUTOUT_CLIP_FEATURES_DOCS_BATCH_START", "STEP609_SHOUTOUT_CLIP_FEATURES_DOCS_BATCH_END")
    )
  }
)
...
$requiredReportGroups = @(
  ...
  @{ logical = "STEP611B fixed final completion verification"; candidates = @("step611b_fixed_final_completion_verification_summary.txt") },
  @{ logical = "STEP611C missing marker group triage"; candidates = @("step611c_missing_marker_group_triage_summary.txt") }
)
...
function Normalize {
  param([string]$Text)
  if ($null -eq $Text) { return "" }
  return ([string]$Text).ToUpperInvariant()
}
...
foreach ($d in $expectedDocs) {
  ...
  if ($exists) {
    $content = Normalize (Get-Content -LiteralPath $full -Raw -Encoding UTF8)

    foreach ($group in $d.markerGroups) {
      $allPartsFound = $true
      $matchedParts = @()
      foreach ($part in $group) {
        $p = Normalize ([string]$part)
        if ($content.Contains($p)) {
          $matchedParts += [string]$part
        } else {
          $allPartsFound = $false
        }
      }
      ...
      if ($allPartsFound) {
        $matchedGroups += (@($matchedParts) -join " + ")
      } else {
        $missingGroups += (@($group) -join " + ")
      }
    }
  } else {
...
$summaryPath = Join-Path $OutDir "step611d_fixed_final_completion_verification_v2_summary.txt"
$docChecksTsv = Join-Path $OutDir "step611d_doc_marker_checks.tsv"
$reportChecksTsv = Join-Path $OutDir "step611d_report_group_checks.tsv"
$mdPath = Join-Path $OutDir "step611d_fixed_final_completion_verification_v2.md"
$jsonPath = Join-Path $OutDir "step611d_fixed_final_completion_verification_v2.json"
...
$md.Add("# STEP611D Fixed Final Completion Verification v2")
$md.Add("")
...
$md.Add("")
if ($completionOk) {
  $md.Add("Die Modul-/Routen-Doku-Batch-Reihe STEP591 bis STEP611D ist abgeschlossen.")
  $md.Add("")
...
$summary = @()
$summary += "STEP611D Fixed Final Completion Verification v2 Summary"
...
Write-Host ""
Write-Host "STEP611D Fixed Final Completion Verification v2 fertig."
