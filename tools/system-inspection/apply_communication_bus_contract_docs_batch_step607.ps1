...
$targetDocRel = "docs/system-inspection/COMMUNICATION_BUS_CONTRACT_CONSOLIDATION.md"
...
if (-not (Test-Path -LiteralPath $nextRowsTsv)) {
  $errors += "Missing input file: step606_next_real_module_doc_batch_rows.tsv"
}
...
$sectionMarker = "<!-- STEP607_COMMUNICATION_BUS_CONTRACT_BATCH_START -->"
$sectionEnd = "<!-- STEP607_COMMUNICATION_BUS_CONTRACT_BATCH_END -->"

$section = New-Object System.Collections.Generic.List[string]
$section.Add("")
$section.Add($sectionMarker)
$section.Add("")
$section.Add("## STEP607 Communication Bus Contract Module Docs Batch")
$section.Add("")
$section.Add("Stand: 2026-05-30")
$section.Add("")
$section.Add("Dieser Abschnitt dokumentiert den Communication-Bus-/EventBus-Anteil aus dem von STEP606 bestimmten naechsten Modul-Doku-Batch.")
$section.Add("")
$section.Add("Der Communication Bus bleibt eine zentrale Status-, Event-, Diagnose- und ACK-Schicht. Dieser Doku-Batch aendert keine produktiven Flows.")
$section.Add("")
$section.Add("### Batch")
$section.Add("")
...
$section.Add("Quelle: system-scan-output/step606_next_real_module_doc_batch_rows.tsv")
$section.Add("Generated: " + $timestamp)
$section.Add("")
$section.Add("### Arbeitsregel")
$section.Add("")
$section.Add("1. Diese Eintraege sind scan-/triagebasiert.")
$section.Add("2. Communication-Bus-/EventBus-Routen duerfen nur als Doku-Hinweis verstanden werden, nicht als Umbauanweisung.")
$section.Add("3. Produktive Flows duerfen nicht ungeprueft auf den Bus umgestellt werden.")
$section.Add("4. ACK-/Status-/Diagnose-Vertraege muessen vor jeder Code-Aenderung separat geprueft werden.")
$section.Add("5. Keine produktive Route ungeprueft aus Scan-Treffern ableiten.")
$section.Add("")
$section.Add("### Communication-Bus-nahe Module in diesem Batch")
$section.Add("")
...
$section.Add("")
$section.Add("Dieser Abschnitt ist eine Doku-/Review-Zwischenablage fuer Communication-Bus-nahe Scan-Kandidaten.")
$section.Add("")
$section.Add("Er ersetzt keine technische Vertragspruefung und keine Entscheidung, ob ein produktiver Ablauf ueber EventBus/Communication Bus laufen soll.")
$section.Add("")
$section.Add("### Naechster Schritt")
$section.Add("")
$section.Add("STEP608 - Communication Bus Batch Verification")
$section.Add("")
$section.Add("Ziel: Pruefen, ob der STEP607-Abschnitt sauber in der Communication-Bus-Konsolidierungsdoku steht und danach den naechsten Modul-Doku-Batch bestimmen.")
$section.Add("")
...
if (@($errors).Count -eq 0) {
  if (Test-Path -LiteralPath $targetDocPath) {
    ...
    $content = "# Communication Bus Contract Consolidation`n"
  }
...
$summaryPath = Join-Path $OutDir "step607_communication_bus_contract_docs_batch_summary.txt"
$rowsOutTsv = Join-Path $OutDir "step607_communication_bus_contract_docs_batch_rows.tsv"
$jsonPath = Join-Path $OutDir "step607_communication_bus_contract_docs_batch.json"
...
$summary = @()
$summary += "STEP607 Communication Bus Contract Module Docs Batch Summary"
...
Write-Host ""
Write-Host "STEP607 Communication Bus Contract Module Docs Batch fertig."
