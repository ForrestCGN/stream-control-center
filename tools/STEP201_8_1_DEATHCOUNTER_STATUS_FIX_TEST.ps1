...
$logPath = "D:\gpt\STEP201_8_1_DEATHCOUNTER_STATUS_FIX_TEST_$timestamp.log"
...
Write-Log "STEP201.8.1 Deathcounter Status Fix Test"
...
$summary = @($results | Where-Object { $_ -is [System.Collections.IDictionary] } | ForEach-Object {
  [ordered]@{
    name = $_.name
    method = $_.method
    url = $_.url
...
$output = [ordered]@{
  ...
  purpose = "STEP201.8.1 Deathcounter Status Fix Test"
