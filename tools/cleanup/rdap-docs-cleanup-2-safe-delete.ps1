...
$StepName = 'RDAP_DOCS_CLEANUP_2_DEV_SAFE_DELETE_AND_MERGE'
$RepoRoot = (Get-Location).Path

if (-not (Test-Path -LiteralPath (Join-Path $RepoRoot 'project-state'))) {
    throw 'Abbruch: project-state/ nicht gefunden. Script aus D:\Git\stream-control-center starten.'
}

if (-not (Test-Path -LiteralPath (Join-Path $RepoRoot '.git'))) {
    throw 'Abbruch: .git nicht gefunden. Script aus dem lokalen Repo-Root starten.'
}
...
$LogDir = Join-Path $RepoRoot ('_handoff/' + $StepName)
...
$JsonPath = Join-Path $LogDir ("cleanup2_" + $Mode.ToLowerInvariant() + "_" + $Timestamp + ".json")
$TxtPath = Join-Path $LogDir ("cleanup2_" + $Mode.ToLowerInvariant() + "_" + $Timestamp + ".txt")
...
$Summary = [pscustomobject]@{
    ...
    present = ($Results | Where-Object { $_.existsBefore }).Count
    missing = ($Results | Where-Object { -not $_.existsBefore }).Count
    deleted = ($Results | Where-Object { $_.action -eq 'deleted' }).Count
    wouldDelete = ($Results | Where-Object { $_.action -eq 'would_delete' }).Count
    logJson = $JsonPath
    logText = $TxtPath
}
...
if (-not $Execute) {
    Write-Host ''
    Write-Host 'Dry-Run fertig. Wenn die Liste passt, echte Loeschung ausfuehren mit:'
    Write-Host '.\tools\cleanup\rdap-docs-cleanup-2-safe-delete.ps1 -Execute'
}
