...
$logPath = "D:\gpt\STEP201_12B_1_TWITCH_PRESENCE_INTEGRATION_FIX_TEST_$timestamp.log"
...
Write-Log "STEP201.12b.1 Twitch-Presence Integration Fix Test"
...
$apiMatrix = [ordered]@{
  ...
  summary = @(
    ...
    [ordered]@{
      ...
      sendValidationStillAvailable = (($results | Where-Object { $_.name -eq "presence api send empty" } | Select-Object -First 1).ok)
...
$output = [ordered]@{
  ...
  purpose = "STEP201.12b.1 Twitch-Presence Integration Fix Test"
