...
function writeIfChanged(file, content) {
  const old = read(file);
  if (old === content) {
    console.log(`[STEP274A1] unverändert: ${path.relative(root, file)}`);
    return false;
  }
  fs.writeFileSync(file, content, 'utf8');
  console.log(`[STEP274A1] aktualisiert: ${path.relative(root, file)}`);
  return true;
}
...
function patchMediaJs() {
  ...
  const newListAssets = String.raw`function listAssets(options = {}) {
  ensureSchema();
  ...
  src = src.replace("step: 'STEP274A'", "step: 'STEP274A1'");
  src = src.replace("note: 'STEP274A ist nur der Core. Dashboard und Command-Anbindung folgen in STEP274B/C.'", "note: 'STEP274A1: Media-Core Fix fuer optionale Listenfilter und ruhigere Scans. Dashboard und Command-Anbindung folgen in STEP274B/C.'");
  src = src.replace("return { name: MODULE_NAME, step: 'STEP274A' };", "return { name: MODULE_NAME, step: 'STEP274A1' };");
...
function patchHelperMediaJs() {
  ...
  src = src.split(execOptionsOld).join(execOptionsNew);
...
try {
  patchMediaJs();
  patchHelperMediaJs();
  console.log('[STEP274A1] Fertig. Bitte node --check ausführen.');
} catch (err) {
  console.error('[STEP274A1] Fehler:', err.message || String(err));
  process.exit(1);
}
