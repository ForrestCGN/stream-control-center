param(
  ...
  [int]$DurationMs = 7000,
  [int]$VisibleMs = 7000,
  [string]$User = "STEP403A_VIP_Visible_Preview"
)

$ErrorActionPreference = "Continue"
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logFile = Join-Path (Get-Location) "STEP403A_vip_overlay_preview_visible_wait_$stamp.log"
...
Log "STEP403A VIP overlay preview visible wait test started"
Log "BaseUrl=$BaseUrl"
...
$requestId = "step403a-visible-" + (Get-Date -Format "yyyyMMddHHmmss")
...
Log "ShowEmit ok=$(Get-Prop $show 'ok' '') eventId=$showEventId deliveredCount=$(Get-Prop $show.result 'deliveredCount' 0) deliveredTo=$($showDeliveredTo -join ',')"
...
Log "VISIBLE_WAIT_DONE sending hide now"
...
Log "HideEmit ok=$(Get-Prop $hide 'ok' '') eventId=$hideEventId deliveredCount=$(Get-Prop $hide.result 'deliveredCount' 0) deliveredTo=$($hideDeliveredTo -join ',')"
...
if ($ok) {
  Log "STEP403A_STATUS=PASS"
  exit 0
} else {
  Log "STEP403A_STATUS=CHECK_REQUIRED"
  Log "LogFile=$logFile"
  exit 1
}
