# Files

Stand: 2026-06-27

## Geaendert in Version 0.2.9 - Dashboard-v2 Navigation angeglichen

```text
frontend/dashboard-v2/src/app/navigation.js
frontend/dashboard-v2/src/app/moduleRegistry.js
frontend/dashboard-v2/src/modules/overview/OverviewPage.jsx
htdocs/dashboard-v2/index.html
htdocs/dashboard-v2/assets/*
docs/current/LOCAL_DASHBOARD_REPLACEMENT_PLAN_CURRENT.md
docs/current/START_HERE_FOR_NEW_CHAT.md
docs/current/NEXT_CHAT_PROMPT_RDAP_REMOTE_MODBOARD_NEXT.md
project-state/CURRENT_STATUS.md
project-state/NEXT_STEPS.md
project-state/TODO.md
project-state/FILES.md
project-state/CHANGELOG.md
```

## Geaendert in Version 0.2.8 - Dashboard-v2 Einstieg vorbereitet

```text
frontend/dashboard-v2/src/app/navigation.js
frontend/dashboard-v2/src/layout/AppShell.jsx
frontend/dashboard-v2/src/layout/Sidebar.jsx
frontend/dashboard-v2/src/layout/Topbar.jsx
frontend/dashboard-v2/src/modules/overview/OverviewPage.jsx
frontend/dashboard-v2/src/styles/global.css
htdocs/dashboard-v2/index.html
htdocs/dashboard-v2/assets/*
docs/current/LOCAL_DASHBOARD_REPLACEMENT_PLAN_CURRENT.md
docs/current/START_HERE_FOR_NEW_CHAT.md
docs/current/NEXT_CHAT_PROMPT_RDAP_REMOTE_MODBOARD_NEXT.md
project-state/CURRENT_STATUS.md
project-state/NEXT_STEPS.md
project-state/TODO.md
project-state/FILES.md
project-state/CHANGELOG.md
```

## Geaendert in Version 0.2.7 - Lokaler Dashboard-Ersatz geplant

```text
docs/current/LOCAL_DASHBOARD_REPLACEMENT_PLAN_CURRENT.md
docs/current/START_HERE_FOR_NEW_CHAT.md
docs/current/NEXT_CHAT_PROMPT_RDAP_REMOTE_MODBOARD_NEXT.md
project-state/CURRENT_STATUS.md
project-state/NEXT_STEPS.md
project-state/TODO.md
project-state/FILES.md
project-state/CHANGELOG.md
```

## Geaendert in Version 0.2.6 - Online-Modoberflaeche bereinigt

```text
remote-modboard/backend/server.js
remote-modboard/backend/public/index.html
remote-modboard/backend/public/assets/modules/module-manifest.js
remote-modboard/backend/public/assets/modules/ui/navigation-cleanup.js
docs/current/START_HERE_FOR_NEW_CHAT.md
project-state/CURRENT_STATUS.md
project-state/NEXT_STEPS.md
project-state/TODO.md
project-state/FILES.md
project-state/CHANGELOG.md
```

## Zuletzt geaendert in Version 0.2.5 - Lokales Dashboard vorbereitet

```text
remote-modboard/backend/server.js
remote-modboard/backend/src/routes/status.routes.js
remote-modboard/backend/public/assets/modules/module-manifest.js
remote-modboard/backend/public/assets/languages/de.js
remote-modboard/backend/public/assets/languages/en.js
remote-modboard/backend/public/assets/modules/local-dashboard/stream-pc-status.js
remote-modboard/backend/public/assets/modules/local-dashboard/lan-access.js
remote-modboard/backend/public/assets/modules/local-dashboard/start-env.js
```
