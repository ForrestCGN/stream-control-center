...
## 2026-06-18 – EVS52.16b Dashboard-Auswertungsbutton sichtbar platziert

- Dashboard-Button `Auswertung starten` zusätzlich direkt im sichtbaren Event-Verwalten-Buttonblock platziert.
- Button wird nur gerendert, wenn `finaleEligibility.canStart=true` ist.
- Button zusätzlich im Tab `Aktuelles Event` sichtbar, wenn das ausgewählte/beendete Event auswertbar ist.
- Backend-Finale-Logik unverändert.
...
- Diagnosezaehler `textWordHitChatOutputsBundled` korrigiert: er steigt jetzt bei jeder erzeugten gebuendelten/neutrale Teiltreffer-Chatmeldung.
- `text-runtime/report` ergaenzt bei Satzloesungen zusaetzlich `points`, damit `Select-Object ... points` die vergebenen Satzpunkte korrekt anzeigt.
- Keine Chatquelle, keine Soundlogik, keine Trefferlogik, keine Punktevergabe und keine DB-Struktur geaendert.
