...
RDAP53 ist live sichtbar und bestaetigt.
...
Empfohlene Richtung:
Plan-only oder sehr kleiner Frontend-only Polish, aber nicht blind bauen.

Sinnvoll waere:
- Anzeige `0 Targets` besser erklaeren.
- Klar machen: Auth-Modell liefert aktuell keine `modulePermissions`.
- Optional Diagnose-Zeile:
  - Rollenrechte vorhanden
  - Module-/Targetrechte aktuell leer
...
2. Kurz bestaetigen, dass RDAP53B live dokumentiert ist.
3. RDAP54 kurz planen.
4. Auf `go` warten.
