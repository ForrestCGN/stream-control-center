...
RDAP55 ist live sichtbar und RDAP55B dokumentiert den Live-Befund.
...
- 8 Rechte werden weiter angezeigt:
  - admin.audit.read
  - admin.roles.manage
  - admin.users.manage
...
Sinnvoll ist zuerst zu entscheiden, ob Permission-Read-Details weiter verbessert werden sollen oder ob dieser Strang vorerst abgeschlossen ist.

Moegliche Richtungen:
- bessere Erklaerung einzelner Permission-Keys,
- read-only Rechte-Kategorien,
- UX-/Doku-Plan fuer spaetere Permission-Verwaltung,
- keine Backend-Route, solange `/api/remote/auth/model` reicht,
- keine DB-Migration,
- keine Writes.

Start im neuen Chat:
1. Dateien aus GitHub/dev lesen.
2. Kurz bestaetigen, dass RDAP55B live dokumentiert ist.
3. RDAP56 kurz planen.
4. Auf `go` warten.
