# RDAP Docs Cleanup 5 - docs/current Archiv-Move Manifest

Stand: 2026-06-27

## Zweck

Dieser Step entfernt historische RDAP- und NEXT_CHAT_PROMPT-Dateien aus dem aktiven Startbereich `docs/current/`, ohne Inhalte hart zu löschen.

Die Dateien werden per Script nach `docs/archive/docs-current-cleanup-5/` verschoben.

## Umfang

- Exakte Archiv-Kandidaten: 400
- Quelle: lokale `docs/current/`-Struktur, gegengeprüft gegen den bisherigen Cleanup-Scan/Snapshot.
- Aktion: Move nach Archiv, kein permanentes Löschen.
- Standardmodus: Dry-Run.
- Execute nur mit `-Execute`.

## Nicht enthalten

- Keine JS-/CSS-/HTML-Codeänderungen.
- Keine DB-Änderung.
- Kein Webserver-Deploy.
- Keine automatische Behandlung der `REVIEW_MANUALLY`-Dateien.
- Keine harten Deletes in diesem Step.

## KEEP_CURRENT bleibt in docs/current
