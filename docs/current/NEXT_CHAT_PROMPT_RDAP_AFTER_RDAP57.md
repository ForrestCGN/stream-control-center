...
RDAP57 ist vorbereitet.

Umgesetzt:

```text
Frontend-only Polish in remote-modboard/backend/public/assets/rdap53-permission-read-detail.js.
Effektive Rollen-Rechte werden read-only nach Bereichen gruppiert.
Bereiche: Admin, Agent / Status, Dashboard / Remote, Sonstige Rechte.
...
RDAP57 lokal testen, `stepdone.cmd`, Webserver-Deploy und live bestaetigen.
...
2. Kurz bestaetigen, dass RDAP57 vorbereitet ist.
3. RDAP57 live bestaetigen oder RDAP57B planen.
4. Auf `go` warten.
