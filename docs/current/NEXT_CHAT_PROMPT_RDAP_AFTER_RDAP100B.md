# NEXT CHAT PROMPT - RDAP after RDAP100B
...
RDAP100B dokumentierte den live bestaetigten Nginx/ISPConfig Stand:

```text
- Separater location /agent-ws Block wurde in ISPConfig/Nginx ergaenzt.
- Bestehender location / Block bleibt unveraendert.
- Normaler HTTP-GET /agent-ws erreicht Node HTTP-Router und liefert erwartbar not_found.
- Public WebSocket-Upgrade ohne Secret erreicht Backend-Upgrade-Handler.
- Backend lehnt erwartbar mit missing_connection_proof ab.
- X-SCC-Agent-Runtime: transport-guarded bestaetigt Backend-Handler.
- X-SCC-Agent-Actions: disabled bestaetigt keine Actions.
...
- /api/remote/agent/status waehrend laufendem Agent pruefen.
- Agent lokal stoppen.
...
2. Danach kurzen Plan fuer RDAP101 nennen.
3. Auf mein explizites `go` warten.
4. Keine Runtime-Aktivierung, kein Agent-Start und keine Testbefehle vor `go`.
