...
function renderTopbar(activeModule) {
  return `
    <header class="cgn-topbar" id="topbar">
      ...
      <div class="top-search disabled" aria-label="Suche später verfügbar"><span>⌕</span><input type="search" placeholder="Suche später: Module, Sounds, Overlays …" disabled></div>
      <div class="top-center" aria-label="Lokale Status-Chips"><span class="cgn-chip cgn-chip--ok">0.2.10D</span><span class="cgn-chip cgn-chip--info">Port 8080</span><span class="cgn-chip cgn-chip--ok">Read-only</span><span class="cgn-chip cgn-chip--info">Stream-PC</span></div>
      <div class="top-right"><button class="cgn-icon-button cgn-refresh" type="button" title="Neu laden später" aria-label="Neu laden später" disabled>↻</button><button class="cgn-icon-button cgn-lang" type="button" disabled>DE</button><button class="cgn-icon-button cgn-bell" type="button" title="Read-only gesperrt" aria-label="Read-only gesperrt" disabled>🔒<em>0</em></button><button class="cgn-avatar-button" type="button" disabled><span class="cgn-avatar cgn-avatar-local">F</span><span class="cgn-avatar-copy"><strong>ForrestCGN</strong><small>Lokal</small></span></button></div>
    </header>`;
}
