...
// Connect to your local overlay control websocket (0.0.0.0:8087, endpoint "/")
// Expected inbound messages (JSON):
//   {"op":"fireworks","intensity":5}
//   {"op":"fireworks","intensity":12,"mode":"finale"}
//   {"op":"fireworks","intensity":3,"mode":"burst","duration_ms":8000}
//
// Queue is ON: incoming requests add to a queue and play sequentially.
let FW_QUEUE = [];
let FW_PLAYING = false;

function enqueueFireworks(intensity, mode) {
  intensity = Math.max(1, Math.min(50, Number(intensity) || 1));
  mode = (mode || "burst").toLowerCase();
  FW_QUEUE.push({ intensity, mode });
  if (!FW_PLAYING) {
    FW_PLAYING = true;
    playNextFromQueue();
  }
...
function playNextFromQueue() {
  if (FW_QUEUE.length === 0) {
    FW_PLAYING = false;
    return;
  }
  const job = FW_QUEUE.shift();

  // "finale" => enable the internal finale-mode briefly
  if (job.mode === "finale") {
    try {
  ...
  // Fire N shells over time (instead of instantly spamming)
  const n = job.mode === "finale" ? Math.max(15, job.intensity) : job.intensity;
  const interval = job.mode === "finale" ? 180 : 350; // ms between launches
  let fired = 0;
  ...
  const timer = setInterval(() => {
    ...
    if (fired >= n) {
      ...
      // turn finale off again after a short grace period
      if (job.mode === "finale") {
        setTimeout(() => {
          try {
      ...
      // small pause before next queue item
      setTimeout(playNextFromQueue, 1200);
    }
  }, interval);
}
...
function connectWebsocket() {
  ...
  // Use your overlay-control websocket server
  const ws = new WebSocket("ws://127.0.0.1:8087/");

  ws.onopen = function () {
    // Optional: identify this client so your server can log/route it
    try { ws.send(JSON.stringify({ op: "hello", client: "fireworks-overlay" })); } catch (e) {}
  };
  ...
  ws.onmessage = function (event) {
    ...
    // Fireworks trigger
    if (payload && (payload.op === "fireworks" || payload.type === "fireworks")) {
      const intensity = payload.intensity ?? payload.amount ?? 1;
      const mode = payload.mode ?? "burst";
      enqueueFireworks(intensity, mode);
      return;
    }

    // Optional: allow chat command passthrough
    // {"op":"chat","message":"!fireworks 10"}
    if (payload && payload.op === "chat" && typeof payload.message === "string") {
      const msg = payload.message.toLowerCase();
      const m = msg.match(/!fireworks\s*(\d+)?\s*(finale)?/);
      if (m) {
        const i = m[1] ? parseInt(m[1], 10) : 5;
        const finale = !!m[2];
        enqueueFireworks(i, finale ? "finale" : "burst");
      }
    }
  };
}
