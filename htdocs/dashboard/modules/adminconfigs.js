(function(){
  ...
  const fallbackRegistry = {
    ...
    configs: [
      ...
      { id:'twitch_dashboard_auth', label:'Twitch Login', path:'config/twitch_dashboard_auth.json', category:'Auth', writablePrepared:false, description:'OAuth-/Scope-Vorbereitung fÃ¼r Dashboard-Login und Mod-Rechte.' },
      { id:'backend_general', label:'Backend Allgemein', path:'config/dashboard_backend_general.json', category:'Backend', writablePrepared:false, description:'Vorbereitung fÃ¼r zentrale Backend-Optionen.' },
  ...
  function renderSoundExpertConfig(payload){
    ...
    return `
      <div class="adminconfig-sound-expert">
        ...
        <p class="muted">Diese Werte sind technische Systemwerte. Sie gehÃ¶ren nicht auf die normale Sound-Seite. Speicherung lÃ¤uft Ã¼ber <code>/api/sound/settings</code> nach SQLite.</p>
        ...
        <div class="placeholder-note">
          <strong>Hinweis:</strong> Secrets/Tokens werden hier nicht angezeigt. Ã„nderungen sollten spÃ¤ter mit Rollen/Rechten und Audit-Logging abgesichert werden.
        </div>
  ...
  function renderPreview(id, payload){
    ...
    if (id === 'streamdesk') {
      ...
      return `<div class="adminconfig-preview"><h3>Stream-Desk Vorschau</h3><div class="adminconfig-chip-row"><span>${scenes.length} QuickScenes</span><span>${Object.values(widgets).filter(Boolean).length} aktive Widgets</span><span>${config.twitchLogin?.enabled ? 'Twitch-Login aktiv' : 'Twitch-Login vorbereitet'}</span></div><div class="adminconfig-scene-preview">${scenes.map(s => `<div><strong>${window.CGN.esc(s.label || s.sceneName)}</strong><small>${window.CGN.esc(s.sceneName || '')}${s.confirm ? ' Â· BestÃ¤tigung' : ''}</small></div>`).join('') || '<p class="muted">Keine Szenen im geladenen Payload.</p>'}</div><p class="muted">Diese Szenen bestimmen spÃ¤ter die OBS-Buttons im Stream-Desk. Nur wichtige sichtbare Szenen eintragen, keine _Hilfsszenen.</p></div>`;
    }
    if (id === 'dashboard_logging') {
      return `<div class="adminconfig-preview"><h3>Audit-Logging</h3><div class="adminconfig-chip-row"><span>Retention: ${window.CGN.esc(config.retentionDays ?? 'â€”')} Tage</span><span>${config.logToDatabase ? 'SQLite aktiv geplant' : 'SQLite vorbereitet'}</span><span>${window.CGN.esc(config.databasePath || 'data/sqlite/dashboard_audit.sqlite')}</span></div><p class="muted">Logs werden kompakt und separat geplant. Keine Tokens, keine kompletten API-Antworten.</p></div>`;
    }
    if (id === 'env_strategy') {
    ...
    return `<div class="adminconfig-preview"><h3>Hinweis</h3><p>Diese Config ist vorbereitet. Lesen funktioniert erst vollstÃ¤ndig, wenn <code>dashboard_controlcenter.js</code> im Backend registriert ist.</p></div>`;
  }
  ...
  function render(){
    ...
    root.innerHTML = `
      <div class="module-grid adminconfigs-grid">
        <section class="card glass span-12 adminconfigs-hero"><span class="role-badge">Admin / vorbereitet</span><h2>Admin Configs</h2><p>Zentrale Konfigurationsseite fÃ¼r Stream-Desk, Navigation, Logging, Twitch-Login, Backend-Optionen und Secrets. Rechte sind weiterhin nicht live scharf geschaltet.</p><div class="placeholder-note adminconfigs-warning"><strong>Wichtig:</strong> Schreiben, Secret-Ersetzen und echte RechteprÃ¼fung bleiben gesperrt, bis Auth + Audit-Log aktiv sind.</div></section>
        <section class="card glass span-12"><div class="adminconfigs-layout"><aside><h2>Config-Bereiche</h2><div class="adminconfigs-list">${configs.map(c => `<button class="adminconfig-item ${c.id === state.selectedId ? 'active' : ''}" data-config-id="${window.CGN.esc(c.id)}"><strong>${window.CGN.esc(c.label)}</strong><span>${window.CGN.esc(c.category || 'Config')} Â· ${window.CGN.esc(c.displayPath || c.path || '')}</span></button>`).join('')}</div></aside><section class="adminconfig-detail"><span class="role-badge">${window.CGN.esc(item.category || 'Config')}</span><h2>${window.CGN.esc(item.label || 'Config')}</h2><p>${window.CGN.esc(item.description || '')}</p><div class="adminconfig-meta"><div class="quick-card"><h3>Pfad</h3><p><code>${window.CGN.esc(item.displayPath || item.path || 'â€”')}</code></p></div><div class="quick-card"><h3>Status</h3><p>${state.loading ? 'lÃ¤dt...' : (payload ? 'lesbar' : 'vorbereitet')}</p></div><div class="quick-card"><h3>Schreiben</h3><p>${canEdit ? 'aktiv' : 'noch gesperrt'}</p></div></div>${state.error ? `<div class="placeholder-note bad">${window.CGN.esc(state.error)}</div>` : ''}${renderPreview(state.selectedId, payload)}<textarea class="adminconfig-editor" spellcheck="false" readonly>${window.CGN.esc(payload ? pretty(payload) : pretty({ preparedOnly:true, note:'Backend-Endpunkt oder Datei noch nicht aktiv. Struktur ist vorbereitet.' }))}</textarea><div class="adminconfig-actions"><button type="button" data-reload-config="${window.CGN.esc(state.selectedId)}">Neu laden</button><button type="button" disabled title="Schreiben wird erst mit Auth/RechteprÃ¼fung aktiviert">Speichern vorbereitet</button><button type="button" disabled title="Ã„nderungen werden spÃ¤ter ins Audit-Log geschrieben">Audit-Vorschau vorbereitet</button></div>${state.note ? `<p class="muted">${window.CGN.esc(state.note)}</p>` : ''}</section></div></section>
