(function(){
  ...
  function ruleRow(r, compact){
    ...
    if (compact) {
      return `<tr><td>${pill(type)}</td><td><strong>${esc(r.label)}</strong></td><td>${statusButton}</td><td>${esc(range)}</td><td>${chatBlockInline(r)}</td><td>${assetInline(r.sound_label, r.sound_url)}</td><td>${esc(duration)}</td><td><button class="icon-action-btn edit" data-edit-rule="${esc(r.id)}" title="Regel bearbeiten" aria-label="Regel bearbeiten">✎</button></td></tr>`;
    }
