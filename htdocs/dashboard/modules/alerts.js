    ...
    return `<div class="modal-backdrop" data-close-modal="1"><div class="modal-card glass" data-modal-card="1">
      ...
      <div class="modal-body">
        ...
        <div class="form-section"><h3>Medien & Design</h3><div class="form-grid editor-grid">
          <label class="wide-field">Sound aus Media-Registry<input type="hidden" id="ruleSoundMediaId" value="${esc(soundMediaId)}"><div class="sound-select-row"><input id="ruleSoundMediaInfo" value="${esc(soundMediaLabel)}" readonly><button type="button" id="pickRuleSoundMedia">MediaPicker</button><button type="button" id="clearRuleSoundMedia" ${soundMediaId ? '' : 'disabled'}>×</button></div></label>
          <label class="wide-field">Legacy-Sound / Fallback<div class="sound-select-row"><select id="ruleSound">${assetOptions('sound', r.sound_asset_id)}</select><button type="button" id="playRuleSound" class="sound-icon-btn sound-select-play" ${selectedSoundUrl(r.sound_asset_id) ? '' : 'disabled'} data-sound-id="${esc(r.sound_asset_id ?? '')}" title="Ausgewählten Legacy-Sound abspielen" aria-label="Ausgewählten Legacy-Sound abspielen">▶</button></div></label>
        ...
        </div><p class="small-note">Media-Registry-Sound hat Vorrang. Legacy-Sound bleibt als Fallback erhalten. Grafik, Rahmen, Innenlinie und Celebration kommen aus dem gewählten Design-Profil.</p></div>
