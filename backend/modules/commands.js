...
function commandResultChatMessage(result) {
  const data = commandResultData(result);
  if (!data) return '';
  return cleanText(data.chatMessage || data.streamerbot_message || data.message || data.text || '');
}

function commandResultChatSendEnabled(command, result) {
  const config = command && command.config && typeof command.config === 'object' ? command.config : {};
  const data = commandResultData(result) || {};
  if (data.send === false || data.chatOutput === false || data.sendChat === false) return false;
  if (String(data.streamerbot_send || '').trim() === '0') return false;
  if (config.disableResultChatSend === true || config.noResultChat === true) return false;
  return config.sendResultToChat === true
    || config.resultChatSend === true
    || config.sendChatReply === true
    || config.chatReply === true
    || config.commandResultChat === true
    || config.chatOutput === true
    || data.send === true
    || data.sendChat === true
    || data.chatOutput === true
    || String(data.streamerbot_send || '').trim() === '1';
}

async function maybeSendCommandResultChatReply(command, parsedCommand, user, result, source = {}) {
  if (!commandResultChatSendEnabled(command, result)) {
    state.chatReplies.skipped += 1;
    state.chatReplies.lastSkippedReason = 'not_enabled';
    return { attempted: false, skipped: true, reason: 'not_enabled' };
  }

  const message = commandResultChatMessage(result);
  if (!message) {
    state.chatReplies.skipped += 1;
    state.chatReplies.lastSkippedReason = 'empty_message';
    return { attempted: false, skipped: true, reason: 'empty_message' };
  }

  state.chatReplies.attempted += 1;
  try {
    const twitchPresence = require('./twitch_presence');
    if (!twitchPresence || typeof twitchPresence.sendChatMessage !== 'function') {
      throw new Error('twitch_presence_sendChatMessage_unavailable');
    }

    const sendResult = await Promise.resolve(twitchPresence.sendChatMessage(message, {
      trigger: `command:${command.trigger}`,
      source: 'commands',
      command: command.trigger,
      aliasTrigger: parsedCommand.trigger || command.trigger,
    ...
    if (sendResult && sendResult.ok === true) {
      state.chatReplies.sent += 1;
      state.chatReplies.lastSentAt = nowIso();
      state.chatReplies.lastError = '';
      state.chatReplies.lastMessagePreview = cleanText(sendResult.message_preview || message).slice(0, 160);
      return {
        attempted: true,
        ok: true,
        sent: true,
        targetModule: 'twitch_presence',
        sentAt: sendResult.sent_at || state.chatReplies.lastSentAt,
        messagePreview: sendResult.message_preview || state.chatReplies.lastMessagePreview,
        sendCount: sendResult.send_count || null
      };
    }

    const error = cleanText(sendResult && (sendResult.error || sendResult.reason) || 'chat_send_failed');
    state.chatReplies.failed += 1;
    state.chatReplies.lastError = error;
    return {
      attempted: true,
      ok: false,
      sent: false,
...
async function processMessage(input = {}, options = {}) {
  ...
  try {
    ...
    const chatReply = await maybeSendCommandResultChatReply(command, parsedCommand, user, result, options);
    state.handled += 1;
    state.executed += 1;
    state.lastCommandAt = nowIso();
    state.lastCommand = { trigger: command.trigger, aliasTrigger: match.aliasTrigger, userLogin: user.login, userDisplayName: user.displayName, rawMessage: parsedCommand.rawMessage, resultStatusCode: result.statusCode || null, chatReply };
    state.lastError = '';
    const summarizedResult = summarizeResultForLog(result);
    if (chatReply && chatReply.attempted) summarizedResult.chatReply = chatReply;
    ...
    return { ok: true, command: command.trigger, matchedBy: match.matchedBy, result: summarizedResult, chatReply };
  } catch (err) {
    state.failed += 1;
...
function statusPayload() {
  return {
    ...
    commandResultChatSend: { enabled: state.chatReplies.enabled !== false, targetModule: state.chatReplies.targetModule, attempted: state.chatReplies.attempted, sent: state.chatReplies.sent, skipped: state.chatReplies.skipped, failed: state.chatReplies.failed, lastSentAt: state.chatReplies.lastSentAt, lastSkippedReason: state.chatReplies.lastSkippedReason, lastError: state.chatReplies.lastError, lastMessagePreview: state.chatReplies.lastMessagePreview },
