...
async function playBirthdayIntro(asset, targetContext = {}) {
  ...
  const introPayload = birthdaySoundPayloadForRef(asset.videoFile, 'video'); // STEP275B_PLAY_INTRO_MEDIAID_PAYLOAD
...
async function playBirthdaySong(asset, targetContext = {}) {
  ...
  const cfg = getConfig();
  const songPayload = birthdaySoundPayloadForRef(asset.songFile, 'audio'); // STEP275B_PLAY_SONG_MEDIAID_PAYLOAD
  return internalRequest('POST', '/api/sound/play', soundPlayBase(asset, targetContext, {
    ...songPayload,
...
function buildBirthdaySoundBundle(asset, targetContext = {}) {
  ...
  const items = [];
  if (asset.videoFile) {
    const introPayload = birthdaySoundPayloadForRef(asset.videoFile, 'video'); // STEP275B_BUNDLE_INTRO_MEDIAID_PAYLOAD
  ...
  if (asset.songFile) {
    const songPayload = birthdaySoundPayloadForRef(asset.songFile, 'audio'); // STEP275B_BUNDLE_SONG_MEDIAID_PAYLOAD
    items.push(soundBundleItemBase(asset, targetContext, {
      role: 'song',
      bundleRole: 'song',
      ...songPayload,
...
// STEP274W_FIX4 importBirthdayMediaAsset top-level
async function importBirthdayMediaAsset(payload = {}) {
  const kind = clean(payload.kind || payload.type || '');
  const mediaId = Number(payload.mediaId || payload.media_id || payload.id || 0);
  if (!mediaId) throw new Error('media_id_required');
...
function buildAssetInfo(label, role, relativeFile, fallbackMs = 0, expectedKind = 'audio') {
  ...
  return {
    ...
    webPath: mediaInfo.webPath || (relative && !mediaId ? `/assets/sounds/${relative}` : ''),
